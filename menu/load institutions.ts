...
function _loadInstitutions() {
  ...
  const { cfg_Accounts } = getReferenceRanges(spreadsheet);

  let institutionsSheet = spreadsheet.getSheetByName(INSTITUTIONS_SHEET_NAME);

  if (!institutionsSheet) {
    institutionsSheet = spreadsheet
      .insertSheet()
      .setName(INSTITUTIONS_SHEET_NAME);
    ...
    spreadsheet
      ...
      .getRange("B7:B7")
      .copyFormatToRange(institutionsSheet.getSheetId(), 1, 4, 1, 1);
  ...
  institutionsSheet
    .getRange(
      2,
      1,
      institutionsSheet.getMaxRows(),
      institutionsSheet.getMaxColumns()
    )
    .clear();
  ...
  for (const row of data) {
    ...
    spreadsheet
      ...
      .copyFormatToRange(
        ...
        1,
        4,
        institutionsSheet.getLastRow(),
        institutionsSheet.getLastRow()
      );

    if (ADD_LOGOS) {
      institutionsSheet.setRowHeight(
        institutionsSheet.getLastRow(),
        LOGO_MAX_HEIGHT
      );

      if (row.logo) {
        const logo = institutionsSheet.insertImage(
          row.logo,
          4,
          institutionsSheet.getLastRow()
        );
